...
def calculate_score(scores):
    ...
    total = 0
    for score in scores:
      if score not in DICE_VALUES: raise ValueError('wrong dice value')
      if score >= MIN_SCORE:
        total += score
    return total
...
def get_winner(players):
    """Given a list of Player namedtuples return the player
       ...
       output:
         Player(name='player 3', scores=[4, 5, 1, 2])
    """
    score_len = len(players[0].scores)
    if not all(len(player.scores) == score_len for player in players):
      raise ValueError
    sorted_players = sorted(players, 
                            key=lambda x: calculate_score(x.scores), 
                            reverse=True)
