...
def person_max_bmi(data=data):
    ...
    max_bmi, char_name = 0, None
    Character = namedtuple('Character', 'name height mass')
    for entry in data.splitlines():
       name, height, mass = entry.strip().split(',')
       char = Character(name, float(height), float(mass))
       bmi = get_bmi(char)
       if bmi > max_bmi:
          max_bmi = bmi
          char_name = char.name
    return char_name, max_bmi
